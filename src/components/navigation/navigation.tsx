  ...
  return (
    <div>
      <Navbar bg="primary" expand="lg">
        <Container fluid>
          {loggedInUser ?
            <Navbar.Brand><Link style={{ color: "#ffffff", paddingRight:"20px", textDecoration: 'none' }} to={loggedInUser.roleflg===Role.Admin ? RouterPath.EngineerList : RouterPath.MyPage }>スキケン</Link></Navbar.Brand>
            : 
            <Navbar.Brand><Link style={{ color: "#ffffff", paddingRight:"20px", textDecoration: 'none' }} to={RouterPath.Login}>スキケン</Link></Navbar.Brand>
          }
              ...
              { loggedInUser?.roleflg===Role.Admin && 
                <div>
                    ...
                    <Link to={RouterPath.PasswordReset} style={{ textDecoration: 'none', color: "#ffffff", paddingRight:'12px', paddingLeft:'2px' }}>
                      <FaBell></FaBell>
                      {notificationCount > 0 && (
                        <span className = "notibell">{notificationCount}</span>
                      )}
                    </Link>
                </div>
              }       
            {loggedInUser && <Nav style={{ color: "#ffffff" }}>ユーザー名&nbsp;:&nbsp;{loggedInUser.name}&nbsp;&nbsp;&nbsp;</Nav>}            
            <Nav>
              {
                loggedInUser ?
                <span onClick={logout1} style={{ color: "#ffffff", paddingRight:"31px", cursor: "pointer" }}>ログアウト <FaSignOutAlt/></span>  
                :
                window.location.pathname !== RouterPath.Login && <Link to={RouterPath.Login} style={{ color: "#ffffff", paddingRight:"31px" }}>ログイン <FaSignInAlt/></Link>                                 
              }
            </Nav>
          </Navbar.Collapse>
        </Container>
