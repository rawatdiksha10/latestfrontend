...
function SkillSet() {
  ...
  const [hourlywages, setHourlyWages] = useState(undefined);
  ...
  function getValidateProfile(profile:any){
    ...
    profile.qualifications = profile.qualifications.filter((x:any) => (x.name !== "" && x.source !== "" && x.acqdate !== null)
    || (x._id !== "" && x.name === "" && x.source === "" && x.acqdate === null));
    return profile;
  }
  ...
  function validateProfile(profile:any) {
    ...
    if(!profile.hourlywages){
      if (!errors.some((error) => error.error_id === "er_hourlywages")) {
        errors.push({ error_id: "er_hourlywages", error_message: "時給を入力してください。" });
      }   
    }
    ...
    //Qualifications
    let partiallyFilledQualifications= profile.qualifications.filter((x:any) => ((x.acqdate !== null && (x.source === "" || x.name === "")) 
    || (x.source !== "" && (x.name === "" || x.acqdate === null)) 
    || (x.name !== "" && (x.acqdate === null || x.source === ""))));
  ...
  async function handleFormControlChanged(event:any) {    
    ...
    if(event.target.id === "hourlywages"){
      setHourlyWages(event.target.value);

      if(event.target.value){
        const filteredErrMsg = errorMessages.filter((error) => error.error_id !== "er_hourlywages");
        setErrorMessages(filteredErrMsg); 
      }else{
        if (!errorMessages.some((error) => error.error_id === "er_hourlywages")) {
          setErrorMessages([...errorMessages, { error_id: "er_hourlywages", error_message: "時給を入力してください。" }]);
        }
      }
    }
      ...
      <div>
        <Container>
          <Row style={{ marginTop: "3%" }}>
            <Col md={6}>
              <Form.Group as={Row}>
                <Form.Label column md={3}>ITE番号:<span style={{ color: 'red' }}>*</span></Form.Label>
                <Col md={7}>
            ...
            <Col md={6}>
              <Form.Group as={Row}>
                <Form.Label column md={4}>名前:<span style={{ color: 'red' }}>*</span></Form.Label>
                <Col md={7}>
          ...
          <Row style={{ marginTop: "3%" }}>
            <Col md={6}>
              <Form.Group as={Row}>
                <Form.Label column md={3}>ステータス:</Form.Label>
                <Col md={7}>
            ...
            <Col md={6}>
              <Form.Group as={Row}>
                <Form.Label column md={4}>時給:<span style={{ color: 'red' }}>*</span></Form.Label>
                <Col md={7}>
                <Form.Control id="hourlywages" size="sm" type="text" value={hourlywages || ''} onChange={handleFormControlChanged}/>
                { getErrorMessage('er_hourlywages') ? <div style={{ color: 'red', fontSize:'small' }} >{getErrorMessage('er_hourlywages')}</div> : null}
                </Col>                      
              </Form.Group>
            </Col>
          </Row>
          ...
          <Row style={{ marginTop: "3%" }}>
            <Col md="6">
              <Form.Group as={Row}>
                <Form.Label column md={3}>実務経験:</Form.Label>
                <Col md={7}>
